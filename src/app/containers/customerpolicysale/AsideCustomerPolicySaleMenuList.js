...
function AsideMenuCustomerPolicySaleMenuList(props) {
    const { layoutProps, menu, intl } = props;
    const { activeMenu, activeSubMenu, setActiveSubMenu, campaign } = useContext(
        GlobalContext
    );

    const [campaignMenuItemList, setCampaignMenuItemList] = useState(subMenus);
...
export default injectIntl(AsideMenuCustomerPolicySaleMenuList);
