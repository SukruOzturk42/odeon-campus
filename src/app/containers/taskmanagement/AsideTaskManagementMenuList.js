...
function AsideTaskManagementMenuList(props) {
  const { layoutProps, menu, intl } = props;
  const { activeSubMenu, setActiveSubMenu } = useContext(GlobalContext);

  const [taskManagementMenuItemList] = useState(menu.subMenuItems);

  useEffect(() => {
    setActiveSubMenu(taskManagementMenuItemList[0]);
  }, []);
    ...
    <>
      {menu.title && (
        <ul className={`menu-nav ${layoutProps.ulClasses}`}>
          ...
          {taskManagementMenuItemList &&
            taskManagementMenuItemList.map((item) => {
              return (
                <li
...
export default injectIntl(AsideTaskManagementMenuList);
